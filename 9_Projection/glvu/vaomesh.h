...
Eigen::Matrix4f ortho(float fovy, float aspect, float zNear, float zFar)
{
    assert(aspect > 0);
    assert(zFar > zNear);

    float radf = fovy / 180 * M_PI;
    float halfHeight = zNear * tan(radf / 2);
    float halfWidth = halfHeight * aspect;

    Eigen::Matrix4f res = Eigen::Matrix4f::Identity();
    res(0, 0) = 1 / halfWidth;
    res(1, 1) = 1 / halfHeight;
...
struct GLMesh
{
    ...
    std::array<R, 16> matMVP(const int *vp, bool offsetDepth = false, bool colmajor = false) const {
        ...
        Mat4 proj = ortho(175, vp[2] / float(vp[3]), 0.1f, 100.0f);//ortho(-2.0f, 2.0f, -2.0f, 2.0f, -100.0f, 100.f); 
    ...
    void draw(const int *vp) 
    {
        ...
        prog.setUniform("MVP", matMVP(vp).data());//这里用了一下
        ...
        prog.setUniform("MVP", matMVP(vp,true).data());//这里也用了一下
        if (showWireframe()) {
